...
import os
import numpy as np
from transformer import transformer_proj
...
class HyperNet(nn.Module):
    def __init__(self, block, layernum, gamma,size):
        # layernum: [127, 64, 32]
        super(HyperNet, self).__init__()
        if layernum is None:
            layernum = [127, 64, 128, 64]
        self.conv1 = self._make_layer(block, layernum[0], layernum[1])#layernum[0]=154, layernum[1]=77
        ...
        self.fuse2 = nn.Sequential(nn.Conv2d(layernum[1], layernum[1], kernel_size=3, padding=1, bias=False),
                                   nn.BatchNorm2d(layernum[1]))
        self.encoder=transformer_proj(size)
        #self.proj=transformer_proj(size)
        ...
        self.tgamma=nn.Parameter(torch.zeros(1))
        ###########################
    ...
    def sprectral(self, x):
        ...
        return b1
    def temporal(self,x1, x2):
        m_batchsize, C, height, width = x1.size()
        #print("x1.size()",x1.size())
        proj_query = x2.contiguous().view(m_batchsize, C, -1)
        proj_key = x2.contiguous().view(m_batchsize, C, -1).permute(0, 2, 1)
        energy = torch.bmm(proj_query, proj_key)
        energy_new = torch.max(energy, -1, keepdim=True)[0].expand_as(energy)-energy
        attention = self.softmax(energy_new)
        proj_value = x1.contiguous().view(m_batchsize, C, -1)

        out = torch.bmm(attention, proj_value)
        out = out.contiguous().view(m_batchsize, C, height, width)

        out = self.tgamma*out + x1
        return out
    ...
    def test(self, X1, X2):
        ...
        f1 = torch.cat([self.fuse1(self.spatial(X1)), self.fuse2(self.sprectral(X1)), self.temporal(X1, X2)], dim=1)
        f2 = torch.cat([self.fuse1(self.spatial(X2)), self.fuse2(self.sprectral(X2)), self.temporal(X2, X1)], dim=1)
        return f1.detach().cpu(), f2.detach().cpu()

    def forward(self, X1, X2, idx):
        if self.training:
            X1_e=self.encoder(X1)
            X2_e=self.encoder(X2)
            #  fuse(avgpool(spatial) + spectral
            z1 = torch.cat([self.fuse1(self.spatial(X1_e)), self.fuse2(self.sprectral(X1_e))], dim=1)
            #z1 = torch.cat([self.fuse1(self.spatial(X1_e)), self.fuse2(self.sprectral(X1_e)), self.temporal(X1_e, X2_e)], dim=1)
            #print('z1 size',np.shape(z1))
            z1 = self.proj(z1)
            z2 = torch.cat([self.fuse1(self.spatial(X2_e)), self.fuse2(self.sprectral(X2_e))], dim = 1)
            #z2 = torch.cat([self.fuse1(self.spatial(X2_e)), self.fuse2(self.sprectral(X2_e)), self.temporal(X1_e, X2_e)], dim = 1)
            #print('z1 size',np.shape(z1))
            z2 = self.proj(z2)
            p1 = self.pred(z1) 
            #print('p1 size',np.shape(p1)) # [N, 24, H, W]
            p2 = self.pred(z2) 
            #print('p2 size',np.shape(p2)) # [N, 24, H, W]
            ...
            p2 = p2.permute([0, 2, 3, 1]).view([-1, p2.shape[1]])  
...
class projector_C(nn.Module):
    def __init__(self, feature_num):
        ...
        self.Conv2d1=nn.Conv2d(feature_num, feature_num,kernel_size=(1, 1), bias=False)
        self.Conv2d3=nn.Conv2d(feature_num, feature_num, kernel_size=(3, 3), padding=1,bias=False)
        self.Conv2d5=nn.Conv2d(feature_num, feature_num, kernel_size=(5, 5),padding=2, bias=False)
        self.proj = nn.Sequential(nn.Conv2d(feature_num*3, feature_num, kernel_size=(1, 1), bias=False),
...
class Focal_cos(nn.Module):
    ...
    def forward(self, p1, p2, z1, z2, idx):
        ...
        #L=0.5*-(torch.mean(loss2)+torch.mean(loss1))
